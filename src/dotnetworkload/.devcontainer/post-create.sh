...
installMinikube=${templateOption:installMinikube}

# Start Minikube with Docker driver and expose port 8081
if ${installMinikube}; then 
    minikube start --driver=docker --container-runtime=cri-o

    # Wait for Minikube to be ready
    while (! kubectl cluster-info ); do
    echo "Waiting for Minikube to launch..."
    sleep 2
    done
fi
